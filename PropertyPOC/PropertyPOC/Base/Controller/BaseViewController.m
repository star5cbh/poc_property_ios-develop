...
- (void)viewDidLoad {
    ...
    [self setNavigationBarBackgroundImage:[UIImage imageWithColor:kAppTabBarColor] tintColor:nil textColor:[UIColor whiteColor] statusBarStyle:UIStatusBarStyleLightContent];
//    [self setUpNoNetView];
    self.noNetType = NoNetShowTypeShowBottom;
    
    [IQKeyboardManager sharedManager].enable = YES;
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(checkNetConnection) name:kNetworkChangedNotification object:nil];
    
}

//- (void)setUpNoNetView{
//    self.topNoti = [[TopNotiView alloc] initWithFrame:
//                    CGRectMake(0,Main_Screen_Height - Navigation_Height - TabBar_Height-20, Main_Screen_Width, 44)];
//    [self.view addSubview:self.topNoti];
//    [self.topNoti hide];
//
//    self.noConnectView = [[NoConnectView alloc] initWithFrame:
//                          CGRectMake(0,Navigation_Height,Main_Screen_Width,Main_Screen_Height - Navigation_Height - TabBar_Height)];
//    [self.view addSubview:self.noConnectView];
//    [self.noConnectView hide];
//}
...
//    [self.view bringSubviewToFront:self.topNoti];
...
- (void)setDefaultNavigationBar {
    [self setNavigationBarBackgroundImage:[UIImage imageWithColor:kAppTabBarColor]
...
- (void)setNavigationBarBackgroundImage:(UIImage *)image
    ...
    self.navigationController.navigationBar.titleTextAttributes = @{NSForegroundColorAttributeName:textColor,NSFontAttributeName:BOLDSYSTEMFONT(19)};
...
- (void)setLeftImageNamed:(NSString *)name action:(SEL)action {
    ...
    UIBarButtonItem *backItem = [[UIBarButtonItem alloc] initWithImage:image style:UIBarButtonItemStylePlain target:self action:action];
    backItem.imageInsets = UIEdgeInsetsMake(0, -5, 0, 5);
    
    if (!action) {
        backItem = [[UIBarButtonItem alloc] initWithImage:image style:UIBarButtonItemStylePlain target:self action:@selector(popBackLastController)];
    }
    backItem.tintColor = kAppMainLineAndTextColor;
...
- (void)setRightItemTitle:(NSString *)title action:(SEL)action {
    [self setRightItemTitle:title titleColor:RGBACOLOR(255, 255, 255, 1) action:action];
}
...
- (void)setRightImageNamed:(NSString *)name action:(SEL)action {
    UIImage *image = [UIImage imageNamed:name];
    UIBarButtonItem *rightBtn = [[UIBarButtonItem alloc] initWithImage:image style:UIBarButtonItemStylePlain target:self action:action];
...
- (YYLabel *)badge {
    if (!_badge) {
        ...
        _badge.backgroundColor = kAppMainColor;
    }
    return _badge;
}
